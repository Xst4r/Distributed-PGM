...
\begin{figure}[!htb]
    ...
    \begin{tikzpicture}
        ...
        \Vertex[x=0,y=0,color=red,opacity=0.5,label=$\mathcal{M}^{(i)}$, size=1]{a}
        \Vertex[x=5,y=5,color=tugreen,opacity=0.5,label=$\mathcal{M}^{(j)}$, size=1]{b}
        \Vertex[x=3.5,y=1,color=tugreen,opacity=0.5,label=$\mathcal{M}^{(k)}$, size=1]{c}
        \Vertex[x=4.5,y=2,color=white,opacity=0.2,label=$\mathcal{M}^{(l)}$, size=1]{d}
        \Edge[Direct ,bend=-8.531, label=$\vect{\theta}^{(i)}$](a)(c)
        \Edge[,bend=-8.531](c)(d)
        \Edge[,bend=-8.531](d)(b)
        \Edge[Direct,bend=-8.531, label=$\vect{\theta}^{(i)}$](a)(b)
        \end{tikzpicture}
        \caption[Cap]{Node $i$ sending the local parameter vector to its neighbors.}
    \end{subfigure}
    \begin{subfigure}{.5\textwidth}
        \center
        \begin{tikzpicture}
            \clip (-1,-1) rectangle (6,6);
            \Vertex[x=0,y=0,color=tugreen,opacity=0.5,label=$\mathcal{M}^{(i)}$, size=1]{a}
            \Vertex[x=5,y=5,color=red,opacity=0.5,label=$\mathcal{M}^{(j)}$, size=1]{b}
            \Vertex[x=3.5,y=1,color=red,opacity=0.5,label=$\mathcal{M}^{(k)}$, size=1]{c}
            \Vertex[x=4.5,y=2,color=white,opacity=0.2,label=$\mathcal{M}^{(l)}$, size=1]{d}
            \Edge[Direct ,bend=-8.531, label=$S(\vect{\theta}^{(i)})_k$](c)(a)
            \Edge[,bend=-8.531](c)(d)
            \Edge[,bend=-8.531](d)(b)
            \Edge[Direct,bend=-8.531, label=$S(\vect{\theta}^{(i)})_j$](b)(a)
            \end{tikzpicture}
            \caption[Cap]{The neighbors returning a score based on the message of node $i$}
        \end{subfigure}
    \caption{Example Network with 4 Nodes}
    \label{fig:network}
\end{figure}