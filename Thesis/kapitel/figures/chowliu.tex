\begin{figure}[!htb]
    \center
        \begin{tikzpicture}
            ...
            \end{tikzpicture}
    \caption[]{Weighted graph G=(V,E), where each weight is the mutual information between two random variables. The edges used for the maximum spanning tree are colored orange, which have a total weight of 76. While the original graph is usualyl fully connected, some edges have been omitted for better visibility. We assume that the missing edges each have a weight of 0.}
    \label{fig:mst}
\end{figure}