\begin{figure}[!htb]
    \center
        \begin{tikzpicture}
            ...
            \end{tikzpicture}
    \caption[Two-Pass Belief Propagation on a tree.]{Two pass Belief propagation. In the first pass messages are collected from the leaves towards the designated root s. White nodes are computed first then green nodes and finally the orange root, that has to wait for node r. In the second pass the messages are propagated from the root back to the leaves, starting with the white node followed by green nodes and finally orange nodes.}
