...
% Problem Definition
%Briefly, given a solution space $\Theta$ containing all feasible solutions to some machine learning task, we are trying to find a solution $\bar{\theta} \in \Theta$, from a set of local solutions $\Psi = \5{\theta^{(1)}, \ldots \theta^{(n)}\} \subset \Theta$ on n devices.
%In this case we want to find a suitable probability distribution $\mathbb{P}_{\vect{\theta}}$ that is optimal in $\vect{\theta}$ \wrt an optimization criterion such as the likelihood.
...
%We measure model performance by defining suitable statistics such as classification accuracy, robustness, generalization capability or some error function.
...
%Probabilistic models in particular only require knowledge about the distribution and the estimated parameters, which results in a more compact representation of the original data, while only requiring a fixed amount of memory determined by the structure.
...
Normally, we consider only the quality of a model, training time, and the required amount of data. 
...
%Nodes/Devices that are within the defined threshold stop sending messages until they receive new messages from neighbors that would lead to the local information not satisfying the conditions.
...
%In terms of communication efficiency our goal usually is to reduce the overhead as much as possible, %while still being able to guarantee proper convergence and sufficiently good performance.
